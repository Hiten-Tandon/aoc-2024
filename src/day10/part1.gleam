...
pub fn get_all_trails(
    ...
    grid |> glearray.get(row) |> result.then(fn(x) { x |> glearray.get(col) })
  {
    Ok(x) if x == should_be && should_be > 0 -> {
      ...
      |> set.union(get_all_trails(grid, row, col - 1, should_be - 1))
      |> set.union(get_all_trails(grid, row + 1, col, should_be - 1))
      |> set.union(get_all_trails(grid, row - 1, col, should_be - 1))
    }
    Ok(0) if should_be == 0 -> [#(row, col)] |> set.from_list
    _ -> set.new()
  }
}
...
pub fn main() {
  ...
  |> iterator.map(fn(x: #(Array(Int), Int)) {
    ...
    |> iterator.map(fn(x) {
      let #(ele, ci) = x
      get_all_trails(grid, ri, ci, 9) |> set.size
    })
